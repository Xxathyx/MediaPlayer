...
/**
 * The v1_21_R5 class implements {@link MapUtil}, it can only be defined once if
...
public class v1_21_R5 implements MapUtil {
	...
	public void update(Player player, int id, byte[] buffer) {
		...
		try {
			...
			if(isPaper) {
				sendPacket.invoke(entityPlayer.getClass().getFields()[7].get(entityPlayer), packet);
			}else {
				net.minecraft.server.network.PlayerConnection connection = ((net.minecraft.server.network.PlayerConnection) entityPlayer.getClass().getFields()[6].get(
						((org.bukkit.craftbukkit.v1_21_R5.entity.CraftPlayer)player).getHandle()));
